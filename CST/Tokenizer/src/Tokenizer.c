...
Token* scan_number(Scanner* s)
{
    ...
    if (peek(s) == '0' && (peek_next(s) == 'x' || peek_next(s) == 'X'))
    {
        ...
        while (isxdigit(peek(s)) && num_idx < (int)sizeof(num_buffer) - 1)
        {
            num_buffer[num_idx++] = advance(s);
        }
        num_buffer[num_idx] = '\0';

        long long value = strtoll(num_buffer, NULL, 16);

        char* word = (char*)malloc(32);
        sprintf(word, "%lld", value);
        return token_create(TOKEN_NUMBER, word, line, column);
    }

    /* Decimal integer or float. Scan integer part, then check for `.` or `e/E`. */
    ...
    /* Optional float type suffix: f, F, l, L. Consume but don't include in word. */
    int word_end = s->offset;
    if (is_float && (peek(s) == 'f' || peek(s) == 'F' || peek(s) == 'l' || peek(s) == 'L'))
        advance(s);
    /* Optional integer type suffix: u, U, l, L, ll, LL, ull, etc. */
    else if (!is_float)
    {
        while (peek(s) == 'u' || peek(s) == 'U' || peek(s) == 'l' || peek(s) == 'L')
            advance(s);
        word_end = s->offset;
    }

    int len = word_end - start;
    char* word = (char*)malloc(len + 1);
    memcpy(word, s->src + start, len);
    word[len] = '\0';

    return token_create(is_float ? TOKEN_FLOAT_LIT : TOKEN_NUMBER, word, line, column);
}
