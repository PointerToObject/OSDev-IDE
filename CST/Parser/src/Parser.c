...
AST* create_floatlit_node(const char* text)
{
    AST* node = (AST*)malloc(sizeof(AST));
    node->type = N_FLOATLIT;
    node->data.float_lit.text = _strdup(text ? text : "0.0");
    return node;
}
...
AST* create_decl_node(char* type, char* name, int pointer_level, AST* init, AST* array_size)
{
    ...
    node->data.decl.array_dims = NULL;
    node->data.decl.array_dim_count = 0;
...
/* parse_declaration may return an N_BLOCK when the source had `Type a, b, c;`
 * (multiple declarators sharing one type spec). These helpers unwrap that
 * block at call sites that iterate decls one-by-one. */
static void add_decls_to_program(AST* prog, AST* d)
{
    if (!d) return;
    if (d->type == N_BLOCK) {
        for (size_t i = 0; i < d->data.block.count; i++)
            program_add_global(prog, d->data.block.statements[i]);
        free(d->data.block.statements);
        free(d);
    } else {
        program_add_global(prog, d);
    }
}

static void append_decls_to_array(AST*** arr, size_t* count, size_t* cap, AST* d)
{
    if (!d) return;
    if (d->type == N_BLOCK) {
        for (size_t i = 0; i < d->data.block.count; i++) {
            if (*count >= *cap) {
                *cap = (*cap == 0) ? 4 : (*cap * 2);
                *arr = (AST**)realloc(*arr, (*cap) * sizeof(AST*));
            }
            (*arr)[(*count)++] = d->data.block.statements[i];
        }
        free(d->data.block.statements);
        free(d);
    } else {
...
AST* parse_primary(Parser* p)
{
    ...
    if (t.type == TOKEN_FLOAT_LIT)
    {
        advance_token(p);
        return create_floatlit_node(t.word);
    }
    ...
    if (t.type == TOKEN_LPAREN)
    {
        ...
        if (next.type == TOKEN_INT || next.type == TOKEN_CHAR_KW ||
            ...
            next.type == TOKEN_FLOAT || next.type == TOKEN_DOUBLE ||
...
AST* parse_declaration(Parser* p)
{
    ...
    /* Parse first declarator. Then while we see `,` keep parsing more declarators
     * with the same type/qualifiers — supports C's `Type a, b, c;` form.
     * Result: a single N_DECL when there's just one, or an N_BLOCK of decls
     * when there are multiple. Callers that iterate decls (parse_program globals,
     * struct members) unwrap the block. */
    AST* first_decl = NULL;
    AST* multi_block = NULL;

    while (1)
    {
        /* Pointer stars on this declarator. The leading `*`s before the first
         * name were already consumed; for subsequent declarators (after `,`)
         * each one gets its own pointer level. */
        int decl_ptr = (first_decl == NULL) ? ptr_level : 0;
        if (first_decl != NULL) {
            while (match_token(p, TOKEN_STAR)) decl_ptr++;
        }

        Token name_tok = expect(p, TOKEN_IDENTIFIER);
        char* name_str = _strdup(name_tok.word);

        /* Accumulate `[size]` dims (multi-dim allowed per declarator). */
        AST**  dims = NULL;
        size_t dim_count = 0, dim_cap = 0;
        while (match_token(p, TOKEN_LBRACKET))
        {
            AST* dim_expr = NULL;
            if (!check_token(p, TOKEN_RBRACKET))
                dim_expr = parse_expression(p);
            expect(p, TOKEN_RBRACKET);
            if (dim_count >= dim_cap) {
                dim_cap = dim_cap == 0 ? 2 : dim_cap * 2;
                dims = (AST**)realloc(dims, dim_cap * sizeof(AST*));
            }
            dims[dim_count++] = dim_expr;
        }
        AST* array_size = (dim_count > 0) ? dims[0] : NULL;

        AST* init = NULL;
        if (match_token(p, TOKEN_ASSIGN))
            init = parse_expression(p);

        AST* node = create_decl_node(type_str, name_str, decl_ptr, init, array_size);
        node->data.decl.array_dims = dims;
        node->data.decl.array_dim_count = dim_count;
        node->data.decl.is_static = is_static;
        node->data.decl.is_extern = is_extern;
        node->data.decl.is_volatile = is_volatile;
        node->data.decl.is_const = is_const;
        ...
        if (first_decl == NULL) {
            ...
            if (multi_block == NULL) {
                multi_block = create_block_node();
                block_add_statement(multi_block, first_decl);
            }
            block_add_statement(multi_block, node);
        }
    ...
    expect(p, TOKEN_SEMICOLON);
    free(type_str);   /* duplicated into each create_decl_node above */

    return multi_block ? multi_block : first_decl;
}
...
AST* parse_switch_statement(Parser* p)
{
    ...
    while (!check_token(p, TOKEN_RBRACE) && !check_token(p, TOKEN_EOF))
    {
        if (check_token(p, TOKEN_CASE))
        {
            advance_token(p);  // consume 'case'
            AST* case_val = parse_expression(p);
            expect(p, TOKEN_COLON);

            // Collect statements until next case/default/}
            AST* body = create_block_node();
            while (!check_token(p, TOKEN_CASE) &&
                !check_token(p, TOKEN_DEFAULT) &&
                !check_token(p, TOKEN_RBRACE) &&
                !check_token(p, TOKEN_EOF))
            {
                block_add_statement(body, parse_statement(p));
            }
            ...
            if (case_count >= case_cap)
            {
                ...
                case_values = (AST**)realloc(case_values, sizeof(AST*) * case_cap);
                case_bodies = (AST**)realloc(case_bodies, sizeof(AST*) * case_cap);
            }
            case_values[case_count] = case_val;
            case_bodies[case_count] = body;
            case_count++;
        }
        else if (check_token(p, TOKEN_DEFAULT))
        {
            advance_token(p);  // consume 'default'
            expect(p, TOKEN_COLON);

            // Collect statements until next case/}
            default_body = create_block_node();
            while (!check_token(p, TOKEN_CASE) &&
                !check_token(p, TOKEN_RBRACE) &&
                !check_token(p, TOKEN_EOF))
            {
                block_add_statement(default_body, parse_statement(p));
            }
        }
        else
        {
    ...
    return create_switch_node(expr, case_values, case_bodies, case_count, default_body);
}

AST* parse_do_while_statement(Parser* p)
{
    expect(p, TOKEN_DO);
    AST* body = parse_statement(p);
    expect(p, TOKEN_WHILE);
    ...
    expect(p, TOKEN_RPAREN);
    expect(p, TOKEN_SEMICOLON);
    return create_do_while_node(body, condition);
}
...
AST* parse_statement(Parser* p)
{
    ...
    if (t.type == TOKEN_SWITCH) return parse_switch_statement(p);
    if (t.type == TOKEN_DO) return parse_do_while_statement(p);
...
AST* parse_struct_declaration(Parser* p)
{
    ...
    {
        append_decls_to_array(&members, &count, &cap, parse_declaration(p));
    }
...
AST* parse_typedef(Parser* p)
{
    ...
    if (check_token(p, TOKEN_STRUCT))
    {
        ...
        if (check_token(p, TOKEN_LBRACE))
        {
            ...
            // Return a struct_decl so the members are preserved for codegen.
            // Use the alias name (e.g. "Aiden") as the struct name � this is
            // what the codegen will emit as TYPE Aiden : STRUCT ... END_TYPE.
            return create_struct_decl_node(alias.word, members, count);
        }
        else
        {
...
AST* parse_function(Parser* p)
{
    ...
    if (!check_token(p, TOKEN_RPAREN))
    {
        do
        {
            ...
            // `struct Tag` parameter type: consume the tag name and build "struct Tag".
            char* ptype;
            if (ptok.type == TOKEN_STRUCT) {
                Token tag = expect(p, TOKEN_IDENTIFIER);
                size_t len = strlen("struct ") + strlen(tag.word) + 1;
                ptype = (char*)malloc(len);
                strcpy(ptype, "struct ");
                strcat(ptype, tag.word);
            }
            else {
                // Resolve typedef for parameter type
            ...
            /* Param array dims (multi-dim allowed) */
            AST**  pdims = NULL;
            size_t pdim_count = 0, pdim_cap = 0;
            while (match_token(p, TOKEN_LBRACKET))
            {
                AST* d = NULL;
                if (!check_token(p, TOKEN_RBRACKET))
                    d = parse_expression(p);
                expect(p, TOKEN_RBRACKET);
                if (pdim_count >= pdim_cap) {
                    pdim_cap = pdim_cap == 0 ? 2 : pdim_cap * 2;
                    pdims = (AST**)realloc(pdims, pdim_cap * sizeof(AST*));
                }
                pdims[pdim_count++] = d;
            }
            AST* arr_sz = (pdim_count > 0) ? pdims[0] : NULL;
            ...
            params[pcount]->data.decl.array_dims = pdims;
            params[pcount]->data.decl.array_dim_count = pdim_count;
            pcount++;
        } while (match_token(p, TOKEN_COMMA));
    }
...
AST* parse_program(Parser* p)
{
    ...
    while (peek_token(p).type != TOKEN_EOF)
    {
        ...
        if (t.type == TOKEN_UNION)
        {
            fprintf(stderr,
                "CST error at line %d: `union` is not supported in IEC 61131-3. "
                "Replace with a tagged struct (a `kind` field plus the alternative members as siblings).\n",
                t.line);
            /* Skip through the closing semicolon to keep parsing the rest of the file. */
            int brace_depth = 0;
            while (peek_token(p).type != TOKEN_EOF) {
                Token cur = advance_token(p);
                if (cur.type == TOKEN_LBRACE) brace_depth++;
                else if (cur.type == TOKEN_RBRACE) { if (--brace_depth <= 0) break; }
            }
            /* Optional declarator + trailing semicolon */
            while (peek_token(p).type != TOKEN_EOF && !check_token(p, TOKEN_SEMICOLON))
                advance_token(p);
            match_token(p, TOKEN_SEMICOLON);
            continue;
        }

        if (t.type == TOKEN_STRUCT)
        {
            /* Distinguish `struct Tag { ... };` (definition) from
             * `struct Tag name;` (variable declaration). After `struct Tag`,
             * the definition continues with `{`; the declaration with an
             * identifier or `*`. */
            Token after_tag = peek_ahead(p, 2);
            if (after_tag.type == TOKEN_IDENTIFIER || after_tag.type == TOKEN_STAR)
                add_decls_to_program(prog, parse_declaration(p));
            else
...
void ast_free(AST* node)
{
    ...
    case N_FLOATLIT:   free(node->data.float_lit.text); break;
    ...
    case N_DECL:
        ...
        /* array_size aliases array_dims[0]; free dims separately to avoid double-free */
        for (size_t i = 0; i < node->data.decl.array_dim_count; i++)
            ast_free(node->data.decl.array_dims[i]);
        free(node->data.decl.array_dims);
    ...
    case N_ASM:
        ...
        break;
    case N_SWITCH:
        ast_free(node->data.switch_stmt.expression);
        for (size_t i = 0; i < node->data.switch_stmt.case_count; i++)
        {
            ast_free(node->data.switch_stmt.case_values[i]);
            ast_free(node->data.switch_stmt.case_bodies[i]);
        }
        free(node->data.switch_stmt.case_values);
        free(node->data.switch_stmt.case_bodies);
        ast_free(node->data.switch_stmt.default_body);
        break;
    case N_DO_WHILE:
