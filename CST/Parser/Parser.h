...
typedef enum
{
	...
	N_FLOATLIT        // 1.5, 1e3, 1.5e-3 — text preserved for round-trip
} Nodes;
...
typedef struct
{
	char* text;     /* literal as written: "1.5", "0.001", "1.5e-3" */
} FloatLitNode;
...
typedef struct
{
	...
	int pointer_level;
	AST* init_value;
	AST* array_size;            /* legacy: first dim, set when array_dim_count >= 1 */
	AST** array_dims;           /* dims 0..N-1 (NULL when array_dim_count == 0) */
	size_t array_dim_count;     /* 0 = scalar, 1 = 1D, >1 = multi-dim */
...
typedef struct
{
	AST* expression;         // switch(expr)
	AST** case_values;       // array of case value expressions (N_INTLIT or N_CHAR_LIT)
	AST** case_bodies;       // array of case body blocks
	size_t case_count;
	AST* default_body;       // optional default block (NULL if none)
} SwitchNode;
