#include "Main.h"
#include "CST-Generation/Targets/target.h"

/* FX backend wants the raw preprocessed source so it can scan for
 * `// CST_IO: X0 -> name` pragmas. */
extern void cst_fx_set_source(const char* src);
...
const char* node_type_name(Nodes type)
{
    ...
    case N_FLOATLIT: return "FLOAT_LITERAL";
...
void ast_print(AST* node, int indent)
{
    ...
    case N_INTLIT:
        ...
        break;
    case N_FLOATLIT:
        printf(" %s", node->data.float_lit.text);
        break;
    case N_STRING_LIT:
...
int main(int argc, char** argv)
{
    if (argc < 4 || strcmp(argv[2], "-o") != 0) {
        fprintf(stderr, "Usage: %s <input.c> -o <output_dir> [-target=beckhoff|allen_bradley]\n", argv[0]);
        return 1;
    }
    ...
    const char* pp_target = NULL;
    char* preprocessed = preprocess(src, base_dir, &pp_target);
    free(src);
    ...
    /* Target selection: CLI flag > source pragma > default (beckhoff) */
    const char* target_name = cli_target ? cli_target : pp_target;
    if (!target_name) target_name = "beckhoff";
    const plc_target_t* tgt = plc_target_lookup(target_name);
    if (!tgt) {
        fprintf(stderr, "Unknown target '%s' (valid: beckhoff, allen_bradley, allen_bradley_ll, mitsubishi_fx)\n", target_name);
        free(preprocessed);
        return 1;
    }
    plc_target_set(tgt);
    ...
    // Generate Structured Text (outputs multiple files to directory)
    printf("=== CST CODE GENERATION ===\n");
    cst_fx_set_source(preprocessed);   /* FX backend reads pragmas from here */
    cst_generate(program, output_dir);

    printf("\nStructured Text files written to: %s\n\n", output_dir);
