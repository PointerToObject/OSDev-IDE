...
void codegen_statement(CodeGen* cg, AST* stmt) {
    ...
    {
        int lbl_end = codegen_new_label(cg);
        int lbl_default = codegen_new_label(cg);
        size_t case_count = stmt->data.switch_stmt.case_count;

        // Allocate labels for each case body
        int case_labels[256];
        // Trampoline labels (pop switch value then jump to case body)
        int case_enter_labels[256];
        for (size_t i = 0; i < case_count && i < 256; i++) {
            case_labels[i] = codegen_new_label(cg);
            case_enter_labels[i] = codegen_new_label(cg);
        }

        // Push break context (break jumps to end, continue passes through to outer loop)
        int outer_continue = get_continue_label();
        push_loop(lbl_end, outer_continue);

        emit(cg, "    ; Switch statement");
        // Evaluate switch expression and save on stack
        codegen_expression(cg, stmt->data.switch_stmt.expression);
        emit(cg, "    push eax  ; Save switch value");

        // Emit comparison chain
        for (size_t i = 0; i < case_count && i < 256; i++) {
            emit(cg, "    ; Case %d comparison", (int)i);
            codegen_expression(cg, stmt->data.switch_stmt.case_values[i]);
            emit(cg, "    cmp [esp], eax  ; Compare switch value to case");
        ...
        if (stmt->data.switch_stmt.default_body) {
            ...
            emit(cg, "    jmp .L%d  ; No match, exit switch", lbl_end);
        }

        // Trampolines: pop switch value then jump to case body
        for (size_t i = 0; i < case_count && i < 256; i++) {
            emit(cg, ".L%d:", case_enter_labels[i]);
            emit(cg, "    add esp, 4  ; Pop switch value");
        ...
        // Default body
        if (stmt->data.switch_stmt.default_body) {
            emit(cg, ".L%d:  ; Default body", lbl_default);
            codegen_statement(cg, stmt->data.switch_stmt.default_body);
        }

        emit(cg, ".L%d:  ; Switch end", lbl_end);
        pop_loop();
        break;
    }
    ...
    case N_DO_WHILE:
    {
        ...
        emit(cg, "    ; Do-while loop");
        emit(cg, ".L%d:  ; Do body", lbl_start);
        codegen_statement(cg, stmt->data.do_while_stmt.body);

        emit(cg, ".L%d:  ; Do-while condition", lbl_cont);
        codegen_expression(cg, stmt->data.do_while_stmt.condition);
        emit(cg, "    test eax, eax");
        emit(cg, "    jnz .L%d", lbl_start);
        emit(cg, ".L%d:  ; Do-while end", lbl_end);
